...
public class statServiceImpl implements statService, CommandLineRunner {
    ...
    public List<countcalc> getMostPopularLocations() {
        String sql = "SELECT site.id,location_name,location_city,location_category,count(*) " +
        ...
        List<countcalc> locations = jbdcTemplate.query(sql, (rs, rowNum) -> {
                return new countcalc(rs.getInt("count(*)"),new Location(rs.getInt("id"),rs.getString("location_name"),
    ...
    public List<timecalc> getMostPopularDisciplines() {
        ...
        ArrayList<timecalc> result = new ArrayList<>();
        for(int i = 0; i < 5; i++){
            result.add(bestTime.get(i));
            result.get(i).setTime(result.get(i).gettime()/60000);
        }
        return result;
    }
